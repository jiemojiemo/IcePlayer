...
IceVolButton::IceVolButton(QWidget *parent) :QWidget(parent)
{
	...
	button->SetButtonIcons(icon_button, icon_button_focus);
...
void IceVolButton::setslidervisiable(bool a)
{
	slider->setVisible(a);
}

void IceVolButton::button_clicked()
{
	menu->exec(QCursor::pos());
}

void IceVolButton::increaseVolume()
{
	slider->triggerAction(QSlider::SliderPageStepAdd);
}

void IceVolButton::descreaseVolume()
{
	slider->triggerAction(QSlider::SliderPageStepSub);
}

int IceVolButton::volume() const
{
	return slider->value();
}

void IceVolButton::setVolume(int volume)
{
